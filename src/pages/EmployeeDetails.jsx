...
const EmployeeDetails = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const res = await axiosSecure.get(`/user-details/${param.id}`);
            if (res.status !== 401) {
    ...
    const { _id, bank_account, designation, email, image, name, role, salary, verified } = employee;



    return (
        <ContainerX>
            <div className="my-10 rounded-xl bg-gray-100 p-10 flex flex-col items-center w-full">
                {
                    <Avatar
                        variant="circular"
                        size="xxl"
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 justify-center mt-5 *:border-2 *:p-5 *:rounded-lg">
                    <div className='space-y-4 bg-gray-300 border-gray-900'>
                        <h2>Name</h2>
                    ...
                    <div className='space-y-4 bg-gray-300 border-gray-900'>
                        <h2>Designation</h2>
                        <span className='text-gray-900 font-semibold '>{designation}</span>
                    </div>
                    <div className='space-y-4 bg-gray-300 border-gray-900'>
